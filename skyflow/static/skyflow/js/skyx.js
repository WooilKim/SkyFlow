...
function draw_info() {
    switch (current_dataset) {
        case "NBA":
            ...
            break;
        case "NUMBEO":
            d3.csv("/static/skyflow/data/processed/NUMBEO/NUMBEO_columns_extent.csv").then(function (data) {
                info_each_height = info_height / columnlistfinal.length;
                console.log('detail', data);
                // console.log(data);
                // columnfinaldata = [];
    ...
    switch (opt) {
        case "NBA":
            d3.json("/static/skyflow/data/processed/NBA_fillna_reduced_organized.json", row => typeClean(row)).then(function (data) {
                ...
                columnlistfinal = ['PTS', 'AST', 'STL', 'BLK', 'TRB', 'ORB', 'DRB', '3P%', '3P', 'FG%', 'FG', 'G'];
                // read_skylines("NBA");
                column_orders = columnlistfinal.slice();
                current_year = 1978;
            ...
            break;
        case "NUMBEO":
            d3.json("/static/skyflow/data/processed/NUMBEO/test.json", row => typeClean(row)).then(function (data) {
                original_dataset = data;
                columnlistfinal = ['QoL', 'PP', 'S', 'HC', 'CoL', 'PPI', 'TCT', 'P', 'C'];
                column_orders = columnlistfinal.slice();
                current_year = 2012;
                selected_years = [current_year];
                year_data = d3.range(2012, 2020);
                year_date = d3.range(2012, 2020).map(function (d) {
                    return new Date(d, 1, 1);
                });
                original_dataset.forEach(function (p) {
                    if (Object.keys(tmp).indexOf(p['PlayerID']) < 0) {
                        tmp[p['PlayerID']] = p['City']
                    }
                })
                Object.keys(tmp).forEach(function (key) {
                    players.push([key, tmp[key]])
...
function histogram() {
    switch (current_dataset) {
        case "NBA":
            ...
            break;
        case "NUMBEO":
            d3.json("/static/skyflow/data/processed/NUMBEO/" + current_dataset + "_histogram.json").then(function (data) {
                histogram_data = data;
                drawfilter();
            });
            break;
...
function set_progress_bar() {
    ...
    dataset_select.selectAll('option')
        .data(['Select Dataset', 'NBA', 'NUMBEO'])
        .enter()
        .append('option')
        .style('height', '20px')
        .text(function (d) {
...
function draw_flow() {
    ...
    flow_svg = d3.select("div#flow")
        .select('svg')
        .attr('width', function () {
            return 100 * year_data.length;
        })
        .attr('height', project_div.clientWidth);
    let gap_between_bars = 100;
    ...
    let areaGenerator = d3.area()
        .x(function (d, i) {
            if (d.info === 's')
                ...
                return gap_between_bars * (year_data.indexOf(d.year)) + padding_left + width_of_bars - padding_between - 20 * d.x - gap;
            else if (d.info === 't')
                return gap_between_bars * (year_data.indexOf(d.year)) + padding_left + width_of_bars - padding_between - 20 * d.x;
...
function draw_selected_flow() {
    let g = flow_svg.select('g.selectedplayer')
    init_selected_flow();
    selected_players.forEach(function (p) {
    ...
    g.selectAll('.selectedflow').remove();
    let y = d3.scaleLinear()
        .range([0, project_div.clientWidth - 30]).domain([0, d3.max(sky_filtered_length.map(x => x.total))]);
            ...
            if (selected_flow[y_i][keys[k_i]].length > 0) {
                ...
                for (p_i in selected_flow[y_i][keys[k_i]]) {
                    ...
                    g.append('rect')
                        ...
                        .attr('x', loc.x)
                        .attr('y', loc.y + y(height / selected_flow[y_i][keys[k_i]].length) * p_i)
                        .attr('width', 5)
                        .attr('height', y(height / selected_flow[y_i][keys[k_i]].length))
...
function get_loc_height_selected_flow(y_i, opt) {
    ...
    if (opt === 'skyline') {
        ...
        y = 10 + scaley(sky_filtered[y_i]['skyline'].length);
    } else {
        y = 20 + scaley(sky_filtered[y_i]['skyline'].length + sky_filtered[y_i]['non-skyline'].length);
    }
    return {'x': x, 'y': y};
}
...
function draw_detail_projected(y_i) {
    ...
    //     })
    //     .attr('transform', function (d, i) {
    //         return 'translate(' + (timeline_year_padding + (timeline_each_width + timeline_gap_width) * i) + ',5)';
    //     });
    ...
    //     let scaleX = d3.scaleLinear().domain(column_extents[column]).range([50, 0]);
    //     let axis = d3.axisLeft(scaleX).tickValues(d3.extent(arr)).tickFormat(function (d) {
    //         if (d >= 1000000)
    //             return Math.floor(d / 1000000) + 'M';
    //         else if (d >= 1000)
    //             return Math.floor(d / 1000) + 'k';
    ...
    //     });
    //     let axis2 = d3.axisLeft(scaleX).tickValues(column_extents[column]);
    //     d3.select('g.attr-' + c_i).select('g.axis-x').call(axis);
    //     d3.select('g.attr-' + c_i).append('g').call(axis2.tickSize(-300)
    //         .tickFormat(''));
    // });
    //
...
function draw_detailview() {
    ...
    //     })
    //     .attr('transform', function (d, i) {
    //         return 'translate(' + (timeline_year_padding + i * (timeline_each_width + timeline_gap_width)) + ',5)';
    //     });
    //
    ...
    //
    //
    // let old_bars = attrs.selectAll('.bar')
    //     .data(function (d) {
    ...
    // let sky_bars = attrs.selectAll('.skybar')
    //     .data(function (d) {
    //         let year = d3.select(this.parentNode).datum();
    //         let y_i = year_data.indexOf(year);
    //         // console.log(year, y_i, yearly_filtered);
    ...
    //         // console.log(column, d3.extent(arr));
    //         let scaleX = d3.scaleLinear().domain(column_extents[column]).range([50, 0]);
    //         // let scaleX = d3.scaleLinear().domain(d3.extent(arr)).range([50, 0]);
    //         // console.log()
    //         // d3.scaleLinear().domain(column_extents[attr])
    //         let axis = d3.axisLeft(scaleX).tickValues(d3.extent(arr)).tickFormat(function (d) {
    //             if (d >= 1000000)
    //                 return Math.floor(d / 1000000) + 'M';
    //             else if (d >= 1000)
    ...
    //         timeline_svg.select('.row-' + y).select('.attr-' + c_i).append('g').call(axis2.tickSize(-300)
    //             .tickFormat(''));
    //     })
    // })
}
